      ...
      {showInstallBanner && installedExtensions.length < 2 && (
        <div className="relative rounded-3xl bg-gradient-to-r from-purple-950/90 via-[#18132e] to-indigo-950/90 border border-purple-600/50 p-5 sm:p-6 shadow-2xl flex flex-col md:flex-row items-start md:items-center justify-between gap-5 backdrop-blur-xl animate-fadeIn">
          <div className="flex items-start gap-3.5 min-w-0">
            <div className="p-3 rounded-2xl bg-gradient-to-tr from-purple-600 to-indigo-600 text-white shadow-xl shadow-purple-600/40 shrink-0">
            ...
            <div className="space-y-1 min-w-0">
              ...
              <h3 className="text-base sm:text-lg font-black text-white tracking-tight">
                ¡Instala todas las extensiones para disfrutar de la experiencia completa!
              </h3>
              <p className="text-xs text-purple-200/80 leading-relaxed max-w-2xl">
                Para poder disfrutar de toda la experiencia de Tachiyomi Web con mangas de todos los scans, te recomendamos instalar todas las extensiones predeterminadas con un solo clic.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2.5 shrink-0 self-end md:self-center">
            <button
              ...
              disabled={isInstallingBatch}
              className="px-4 py-2.5 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 text-black font-black text-xs flex items-center gap-2 shadow-lg shadow-amber-500/20 transition transform active:scale-95 cursor-pointer disabled:opacity-50"
            >
              {isInstallingBatch ? (
                <>
                  <Loader2 className="w-3.5 h-3.5 animate-spin" />
                  <span>Instalando todas...</span>
                </>
              ) : (
                <>
                  <Download className="w-3.5 h-3.5" />
                  <span>Instalar todas de golpe</span>
                </>
              )}
            </button>
            ...
            <button
              onClick={() => {
                setShowInstallBanner(false);
                setOnlyInstalledFilter(true);
                localStorage.setItem('tachiyomi_only_installed', 'true');
                localStorage.setItem('tachiyomi_dismiss_ext_banner', 'true');
              }}
              className="px-3.5 py-2.5 rounded-xl bg-gray-900/80 hover:bg-gray-800 border border-gray-700 text-gray-300 hover:text-white text-xs font-semibold transition cursor-pointer"
            >
              Solo las instaladas
            </button>
          </div>
