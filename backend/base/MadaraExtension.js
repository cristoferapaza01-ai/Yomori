...
export class MadaraExtension extends BaseExtension {
  ...
  async getCatalog(page = 1) {
    try {
      ...
      // Intentar primero método Keiyoushi: madara_load_more AJAX
      ...
      // Método estándar: /manga/page/{p}/ o /manga/?m_orderby=latest&page={p}
  ...
  async getMangaDetails(mangaUrl) {
    try {
      ...
      // Extracción de capítulos según la especificación de Keiyoushi
      let chapters = this.parseChaptersFromDOM($);

      // Si no hay capítulos en el DOM estático, invocar el endpoint AJAX oficial de Madara (/ajax/chapters/)
      ...
      // Ordenar descendente (capítulo más reciente primero)
  ...
  async extractChapter(chapterUrl) {
    try {
      ...
      $('#readerarea img, .reading-content img, .page-break img, .wp-manga-chapter-img, .entry-content img, #chapter-content img, .limit img, img[src*="media.manhwaweb.xyz"], img[src*="wp-content/uploads"]').each((_, el) => {
        ...
        src = src.trim();
        if (src.includes(' ')) src = src.split(' ')[0]; // Si era un srcset con múltiples densidades
